const bankingApp = Vue.createApp({
    ...
    computed: {
      accountStatements() {
        if (
          this.selectedAccountStatement &&
          this.statements[this.selectedAccountStatement]
        ) {
    ...
    methods: {
      ...
      openAccount() {
        ...
        axios
          ...
          .then((response) => {
            if (response.data.success) {
              const checkingAccount = this.accounts.find(
                (acc) => acc.name === 'checking'
              );
      ...
      renameAccount() {
        ...
        axios
          ...
          .then((response) => {
            if (response.data.success) {
              const account = this.accounts.find(
                (acc) => acc.name === this.editAccount.name
              );
              if (account) {
      ...
      filterUsers() {
        if (
          !this.manageAccountName ||
          typeof this.manageAccountName.users !== 'string'
        ) {
          this.filteredUsers = [];
          return;
        }
        ...
        if (this.manageUserName === '') {
          ...
          this.filteredUsers = usersArray.filter((user) =>
            user.toLowerCase().includes(this.manageUserName.toLowerCase())
          );
        }
      },
      selectUser(user) {
